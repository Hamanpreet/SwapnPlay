...
const getRequestSend = (sub) => {
  return db
    .query(`SELECT m.id AS Match_Id, m.status AS Match_Status, m.toy_id AS Owner_Toy_Id, m.toy_in_exchange_id AS Requester_Toy_Id, m.owner_id AS Owner_UserId, m.requester_id AS Requester_User_Id, m.created_at AS request_time, 
                  o.title AS Owner_Toy_Title, o.description AS Owner_Toy_Description, o.age_group AS Owner_Toy_AgeGroup, o.value AS Owner_Toy_Value, o.address AS Owner_Toy_Location, o.Longitude AS Owner_Toy_Longitude, o.latitude AS Owner_Toy_Latitude, o.condition AS Owner_Toy_Condition, io.url AS Owner_Toy_Url, 
                  r.title AS Requester_Toy_Title, r.description AS Requester_Toy_Description, r.age_group AS Requester_Toy_AgeGroup, r.value AS Requester_Toy_Value, r.address AS Requester_Toy_Location, r.Longitude AS Requester_Toy_Longitude, r.latitude AS Requester_Toy_Latitude, r.condition AS Requester_Toy_Condition, ir.url AS Requester_Toy_Url 
            FROM match m 
              INNER JOIN users u ON m.requester_id = u.id 
              INNER JOIN toy o ON m.toy_id = o.id 
              INNER JOIN toy r ON m.toy_in_exchange_id = r.id 
              LEFT JOIN image io ON io.toy_id = m.toy_id 
              LEFT JOIN image ir ON ir.toy_id = m.toy_in_exchange_id 
